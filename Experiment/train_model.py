...
from Load_Dataset import RandomGenerator, ValGenerator, get_dataset_loader
...
def main_loop(batch_size=config.batch_size, model_type='', tensorboard=True):
    if os.path.exists(config.train_dataset):
        print("Train dataset is empty:", len(os.listdir(config.train_dataset)) == 0)
    else:
        print("Train dataset directory does not exist.")

    train_tf = transforms.Compose([RandomGenerator(output_size=[config.img_size, config.img_size])])
    val_tf = ValGenerator(output_size=[config.img_size, config.img_size])
    train_dataset = get_dataset_loader(config.train_dataset, train_tf, image_size=config.img_size)
    val_dataset = get_dataset_loader(config.val_dataset, val_tf, image_size=config.img_size)
    ...
    if model_type == 'ACC_UNet':
        ...
        model.load_from()
        lr = 5e-4
    elif model_type == 'MPM_Net':
        model = MMP_Net (n_channels=config.n_channels, n_classes=config.n_labels,
    ...
    model = model.cuda()
    input_shape = (config.batch_size, config.n_channels, config.img_size, config.img_size)
    macs, params = get_model_complexity_info(model, input_shape)
    logger.info(f'Model {model_type} FLOPs: {macs}, Parameters: {params}')

    with open(os.path.join(config.save_path, f'{model_type}_complexity_info.txt'), 'w') as f:
        f.write(f'Model: {model_type}\n')
        f.write(f'FLOPs: {macs}\n')
        f.write(f'Parameters: {params}\n')

    import platform
    logger.info('Training on ' + platform.node())
    ...
    if tensorboard:
        ...
        writer.add_text('Model_Complexity', f'FLOPs: {macs}, Params: {params}', 0)
    else:
        writer = None
